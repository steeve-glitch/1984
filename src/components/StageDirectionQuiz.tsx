...
import { StageDirectionQuestion, StageDirectionOption } from '../types';

interface StageDirectionQuizProps {
  questions: StageDirectionQuestion[];
  onComplete: () => void;
}

const StageDirectionQuiz: FC<StageDirectionQuizProps> = ({ questions, onComplete }) => {
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [selectedOption, setSelectedOption] = useState<StageDirectionOption | null>(null);
  const [isAnswered, setIsAnswered] = useState(false);

  const currentQuestion = questions[currentQuestionIndex];

  const handleOptionSelect = (option: StageDirectionOption) => {
  ...
  const isCorrect = selectedOption?.text === currentQuestion.answer;

  const getButtonClass = (option: StageDirectionOption) => {
    if (!isAnswered) {
...
export default StageDirectionQuiz;
